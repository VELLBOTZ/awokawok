...
const { blocked,hitnya,spamBan,linkgcnya,siminya,antidel } = require("./message/jsonfile");
...
copyright = `© ${fake1}`
welcomeType1 = settings.welcomeType1
welcomeType2 = settings.welcomeType2
welcomeType3 = settings.welcomeType3
...
//AUTO READ   
 ...
 // Auto Read Group 
if(autoreadgc){
var readgc = await xdev.chats.array.filter(v => v.jid.endsWith('g.us'))
readgc.map( async ({ jid }) => {
await xdev.chatRead(jid, "read")
})
}
...
// Auto Read Private 
if(autoreadpc){
var readpc = await xdev.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
//ANTI VIRUS
 ...
 //ANTI LINK GROUP
				...
				if (isGroupAdmins) return reply('Admin grup mah bebas share link yakan :v')
...
//AUTO BIO
    ...
    initeks = `${kyun(runoy)}`
    await xdev.setStatus(`${fake} Aktif Selama: ${initeks} | Baterai: ${baterai.battery}`)
    }
    */
...
break
	...
	if ((args[0]) === 'troli'|| (args[0]) === 'product' ){
		if(bcTroli) return reply("Udah aktif")
		bcTroli = true
        bcLokasi = false
        bcDokumen = false   
		await xdev.sendMessage(from,"Berhasil mengubah broadcast ke troli", text, {quoted: ftroli})
		} else if ((args[0]) === 'lokasi' ){
		if(bcLokasi) return reply("Udah aktif")
		bcTroli = false
        bcLokasi = true
        bcDokumen = false   
		await xdev.sendMessage(from,"Berhasil mengubah broadcast ke lokasi", text, {quoted: ftroli})
		} else if ((args[0]) === 'document' || (args[0]) === 'pdf' || (args[0]) === 'dokumen' || (args[0]) === 'ori'){
		if(bcDokumen) return reply("Udah aktif")
		bcTroli = false
        bcLokasi = false
        bcDokumen = true   
		await xdev.sendMessage(from,"Berhasil mengubah broadcast ke dokumen", text, {quoted: ftroli})
		} else if (!q) {
...
res = await yts(q)
 ...
 views = res.all[0].views
  if (Number(views) <= 1000){
 	var viewsnya = views
 } else if (Number(views) <= 1000000){
 	var viewsnya = views/'1000' 
      var round = Math.round(viewsnya)+ ' Ribu'
 } else  if (Number(views) <= 1000000000){
 	var viewsnya = views/'1000000' 
          viewsnih = views.toLocaleString().split(",")[1]/'100'
 var round = Math.round(viewsnya)+ ',' + Math.round(viewsnih)+' Juta'
 } else  if (Number(views) >= 1000000000){
 	var viewsnya = views/'1000000000' 
          viewsnih = views.toLocaleString().split(",")[1]/'100'
 var round = Math.round(viewsnya)+ ',' + Math.round(viewsnih)+' Milyar'
 }
 let okiu = `    
...
File Video : ${ras[0].size}`
  ...
  case 'playmp3':
if(!q) return reply("Masukan link vidio youtube kak")
               reply('*Downloading*')
               ...
               if(Number(filesize) > 15 ) return reply(`Bjir sizenya ${res[0].size}\nAu ah ga mao download 😤`)
               }
  ...
  case 'playmp4':
            if (!q) return reply('Link youtubenya mana kak ?')
            reply('*Downloading*')
            ...
            if(res[0].size.includes("MB")){
            filesize =`${res[0].size.split(".")[0]}`
            if(Number(filesize) > 45 ) return reply(`Bjir sizenya ${res[0].size}\nAu ah ga mao download 😤`)
            }
        ...
        case 'antibuggc':
           ...
           case 'autoread':
                 ...
                 case 'autoreadgc':
		            if (!isOwner) return onlyOwner()
					if ((args[0]) === 'on' || (args[0]) === 'enable' || (args[0]) === '1' ) {
					if (autoreadgc === true) return reply('Udah aktif kak')
                    autoread = false
                    autoreadgc = true
                    let ih =`Mode auto read gc telah aktif kak \nBot akan auto read saat di chat`
                    xdev.sendMessage(from, ih, text, { quoted: ftext, contextInfo: forward})
					} else  if ((args[0]) === 'off' || (args[0]) === 'disable' || (args[0]) === '0') {
					 if (autoreadgc === false) return reply('Udah off kak')
                     autoread = false
                    autoreadgc = false
                 ...
                 case 'autoreadpc':
		            if (!isOwner) return onlyOwner()
					if ((args[0]) === 'on' || (args[0]) === 'enable' || (args[0]) === '1' ) {
					if (autoreadpc === true) return reply('Udah aktif kak')
                    autoread = false
                    autoreadpc = true
                    let ih =`Mode auto read telah pc aktif kak \nBot akan auto read saat di chat`
                    xdev.sendMessage(from, ih, text, { quoted: ftext, contextInfo: forward})
					} else  if ((args[0]) === 'off' || (args[0]) === 'disable' || (args[0]) === '0') {
					 if (autoreadpc === false) return reply('Udah off kak')
                     autoread = false
                    autoreadpc = false
...
await reply( `Mampus kena kick awokawok 🤣`);
...
case 'readall':
                    ...
                    reply(`Berhasil membaca ${readall.length} pesan`)
					break
...
case 'setallpayment':
        ...
        if (((isMedia && !dev.message.videoMessage) || isQuotedImage || isQuotedSticker) && args.length == 0) {
          boij = isQuotedImage || isQuotedSticker ? JSON.parse(JSON.stringify(dev).replace("quotedM", "m")).message.extendedTextMessage.contextInfo : dev;
          delb = await xdev.downloadMediaMessage(boij);
          fs.writeFileSync(`./kode qr/allpayment.jpg`, delb);
          fakestatus(`Berhasil mengubah kode qr allpayment`)
        } else if(q){
          reply(`Kirim gambar dengan caption ${prefix}setallpayment`);
        }
        break;
...
case 'explain':
                if (!isOwner) return onlyOwner()
                if ((args[0]) === 'on' || (args[0]) === 'enable' || (args[0]) === '1' ) {
                  if(explain) return reply("Udah aktif")
                    explain = true
					reply(`Sukses mengaktifkan explain silakan cek dengan ketik ${prefix}menu`)
                } else  if ((args[0]) === 'off' || (args[0]) === 'disable' || (args[0]) === '0') {
                   if(!explain) return reply("Sudah mati")
                    explain = false
					reply(`Berhasil mematikan explain`)
                } else if (!q) {
...
break
	...
	try{
            ...
            await xdev.sendMessage(from,li,image,{quoted: dev})
					      ...
					      console.log(err)
					        reply("Yah ga ketemu, coba lagi dong ^_^ ")
					       }
            break
...
case 'togif':
               ...
               reply(mess.wait)
               encmediaaa = isQuotedSticker ? JSON.parse(JSON.stringify(dev).replace('quotedM','m')).message.extendedTextMessage.contextInfo : dev
               mediaaa = await xdev.downloadAndSaveMediaMessage(encmediaaa)
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               xdev.sendMessage(from, mp4, video, {mimetype: 'video/gif', quoted: dev, caption: "Nih"})
        ...
        case 'tovidio':
               ...
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               xdev.sendMessage(from, mp4, video, {mimetype: 'video/mp4', quoted: dev, caption: "Nih"})
               fs.unlinkSync(mediaaa)
               } else {
               reply("Format salah")
}
...
await reply(`Asek asek jatah kick euy 😋`);
...
try{
			        ...
			        if (!isBotGroupAdmins) return sendSticker(jadiinadmin)     
					if (args.length < 1) {
						if(!q) return reply("Tag atau Reply atau input nomer target")
					  ...
					  woke = q.replace(new RegExp("[()+-/ +/]", "gi"), "") + `@s.whatsapp.net`
						...
						reply("Tag atau Reply atau input nomer target")
						}
						} catch (err){
						console.log(err)
						reply("Terjadi Error, Tolong input nomer dengan benar")
							}
					break
...
if(!isGroupAdmins && !dev.key.fromMe) return 
            if(!isGroup) return reply(mess.only.group)
            if (isVote) return reply('Sesi Voting Sedanberlangsung')
if (dev.message.extendedTextMessage === null || dev.message.extendedTextMessage === undefined) {
            if(!q) return reply('*Voting*\n\n'+ prefix+ 'voting @tag target | reason  | 1 (1 = 1 Menit)')
if (args[0].includes('08')) return reply('Awali nomor dengan 62')  
split = args.join(' ').split('|')
nomor = split[0].replace(new RegExp("[()+-/ +/]", "gi"), "")


if(!Number(split[1])) return reply('masukan angka di baris ke 3\nContoh: 1-9999\n1 = 1 Menit')
await mentions('Vote ' +'@'+ nomor+' Di Mulai' +'\n\n' + `ketik vote untuk setuju \nketik devote untuk tidak setuju\n-Huruf kecil semua\n\nvote = ✅\ndevote = ❌\n\nAlasan: ${split[1]}`,[id],true)
addVote(from,split[1],nomor,split[2],reply)
...
if (!isGroup) return reply("Hanya bisa di group")
if(!q) return reply(`Contoh: ${prefix}voting reply pesan|orangnya baperan|2 `)
let id = dev.message.extendedTextMessage.contextInfo.participant[0]
split = args.join(' ').replace('@', '').split('|')
if(!Number(split[1])) return reply('masukan angka di baris ke 3\nContoh: 1-9999\n1 = 1 Menit')
await mentions('Vote ' +'@'+ id.split('@')[0]+' Di Mulai' +'\n\n' + `ketik vote untuk setuju \nketik devote untuk tidak setuju\n-Huruf kecil semua\n\nvote = ✅\ndevote = ❌\n\nAlasan: ${split[1]}`,[id],true)
addVote(from,split[0],id,split[1],reply)
...
if (mentioned.length > 1) return reply(`Satu satu dong`)
 ...
 reply(`Kirim perintah ${prefix}voting tag/reply/nomo/|alasan|waktu vonting contoh: ${prefix}voting tag member|orangnya baperan|2 `)
 }
break    
	*/
...
try{
					...
					await xdev.sendMessage(from, hasil, image, {quoted: ftext, caption: '*GELAP BOS :V*'})
					} catch (err){
						reply(err)
						}
				break
...
case "listonline2": 
        ...
        try {
          let online = [...Object.keys(alpha.chats.get(id).presences),xdev.user.jid]
...
case 'listgroup':
  ...
  break
    ...
    try{
                ...
                case 'allpayment':
                  xdev.sendMessage(from, fs.readFileSync('./kode qr/allpayment.jpg'), image, {quoted: ftroli})
                    break 
                case 'gopay':
...
//Jika ada yang tag nomer owner
                  if (!dev.key.formMe && isOwner && isGroup && budy.includes(nomerOwner)) {
              	if (isSpammerr) return 
		          addSpammer(sender, "10s", spamkuyy)
 ...
 gbuttonan = {
        ...
        sendButMessage(from, `KODE QR YANG TERSEDIA`, `Silahkan pilih salah satu`, [
                 ...
                 {buttonId: `${prefix}dana`, buttonText: {displayText: `ᴅᴀɴᴀ`,},type: 1},
                  {buttonId: `${prefix}allpayment`, buttonText: { displayText: `ᴀʟʟ ᴘᴀʏᴍᴇɴᴛ`,},type: 1}]);
                    }
        ...
        if(isExtream) return
        sendKontak(from, "62882397144534", "LUFFYSAN", "Sibuk");
...
Support gua ya gaes dengan subs\n\nhttps://youtube.com/channel/UCcg05vZUl38_jMLUh1BFr_g`,
...
reply(String(e))
    ...
    if(e.includes("ECONNABORTED")) return reply(`]─────「 *SYSTEM-ERROR* 」─────[\n\n${e}\\n\n© ${fake1}`)
